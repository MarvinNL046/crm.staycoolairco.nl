-- SAFEST COMPREHENSIVE DATABASE OVERVIEW
-- This script avoids all enum and column issues by being very conservative
...
-- ========================================
-- 3. TABLE ROW COUNTS AND BASIC INFO
-- ========================================
WITH table_info AS (
    SELECT 
        schemaname,
...
FROM table_info
ORDER BY estimated_rows DESC, tablename;
...
SELECT 
    ...
    t.max_users,
    t.max_leads,
    DATE(t.subscription_ends_at) as subscription_ends_date,
...
SELECT 
    ...
    STRING_AGG(
        CASE 
            WHEN indexname LIKE '%pkey%' THEN '🔑'
            WHEN indexname LIKE '%unique%' THEN '🔒'
            WHEN indexname LIKE '%tenant%' THEN '🏢'
            ELSE '📊'
        END || ' ' || indexname, 
        ', ' 
        ORDER BY indexname
...
-- ========================================
-- END OF SAFEST DATABASE OVERVIEW
-- ========================================