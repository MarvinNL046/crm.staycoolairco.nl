...
SELECT 
    ...
    isz.index_size,
    CASE 
        WHEN iu.idx_scan = 0 THEN 'DROP INDEX IF EXISTS public.' || iu.indexname || ';'
        ELSE '-- Index is used, keeping it'
...
SELECT 
    'DROP INDEX IF EXISTS public.' || indexrelname || '; -- ' || tablename || ' (' || pg_size_pretty(pg_relation_size(indexrelid)) || ')'
    as drop_statement
