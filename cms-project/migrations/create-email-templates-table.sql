-- Create email_templates table for CRM email template management
...
WHERE NOT EXISTS (
    ...
    WHERE et.tenant_id = t.id AND et.template_type = 'lead_welcome'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates et 
    WHERE et.tenant_id = t.id AND et.template_type = 'appointment_confirmation'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates et 
    WHERE et.tenant_id = t.id AND et.template_type = 'invoice_sent'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates et 
    WHERE et.tenant_id = t.id AND et.template_type = 'quote_sent'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates et 
    WHERE et.tenant_id = t.id AND et.template_type = 'follow_up'
);
