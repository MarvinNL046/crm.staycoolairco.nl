...
-- Run this migration to add BTW percentages, tags, and email templates

-- Add BTW Percentages (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'btw_percentages') THEN
        ...
        RAISE NOTICE 'BTW percentages added';
    ELSE
        RAISE NOTICE 'BTW percentages table does not exist';
    END IF;
END $$;

-- Add Tags (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'tags') THEN
        ...
        RAISE NOTICE 'Tags added';
    ELSE
        RAISE NOTICE 'Tags table does not exist';
    END IF;
END $$;

-- Add Email Templates (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'email_templates') THEN
        ...
        RAISE NOTICE 'Email templates added';
    ELSE
        RAISE NOTICE 'Email templates table does not exist';
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Added configuration data for tenant: StayCool Airconditioning';
END $$;