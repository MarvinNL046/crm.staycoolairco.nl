...
CREATE TABLE IF NOT EXISTS campaigns (
  ...
  -- Campaign settings
  type VARCHAR(50) NOT NULL DEFAULT 'email', -- email, sms, etc
  status VARCHAR(50) NOT NULL DEFAULT 'draft', -- draft, scheduled, sending, sent, failed
  
  -- Targeting
  segment_type VARCHAR(50) DEFAULT 'all', -- all, leads, customers, custom
...
CREATE TABLE IF NOT EXISTS campaign_recipients (
  ...
  -- Status tracking
  status VARCHAR(50) DEFAULT 'pending', -- pending, sent, delivered, opened, clicked, bounced, unsubscribed, complained
  sent_at TIMESTAMP WITH TIME ZONE,
...
CREATE INDEX idx_campaign_clicks_recipient_id ON campaign_clicks(recipient_id);
