...
-- Fixed version that works with Supabase auth constraints
...
-- Create super_admins table instead of modifying auth.users
...
-- Update existing policies to respect super admin access
...
BEGIN
  ...
  VALUES (target_user_id, user_email, auth.uid())
  ...
  VALUES (
    ...
    'user',
    target_user_id,
    jsonb_build_object('email', user_email, 'created_by', auth.uid())
  );
END;
